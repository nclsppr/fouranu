...
- Un seul site expert est le bon point de départ. Une ferme de micro-sites disperse l'autorité, le budget éditorial et la maintenance, tout en augmentant les risques de pages satellites, d'affiliation pauvre et de contenu produit à grande échelle.
...
- La meilleure forme de Four à Nu est un média documentaire permanent avec
  données structurées, sources tierces traçables, synthèses originales et outils
  de décision.
- La pizza napolitaine reste le premier territoire éditorial de Four à Nu sans
  limiter la marque à ce style. Elle combine produits visuels, achats de 300 à
  1 500 euros pour l'amateur, matériel professionnel plus cher et plusieurs
  programmes directs. Sa principale difficulté est la concurrence de marchands,
  créateurs spécialisés et grands médias déjà établis.

La décision recommandée est donc de développer un seul média documentaire
spécialisé, puis de mesurer son utilité et son économie avant d'augmenter le
nombre de contenus.
...
2. une méthode publique qui distingue fabricants, tiers, synthèse `FAN-SYN` et
   inférence `FAN-INF` ;
...
mesure tierce reste attribuée à son auteur. Four à Nu exclut les notes, étoiles,
classements pseudo-scientifiques et données structurées d'avis.
...
met cependant en garde contre la simple reformulation de sources. Four à Nu
doit donc apporter une méthode, des rapprochements et des limites propres. Il
n'emploie ni note, ni étoile, ni données structurées `Review` ou
`AggregateRating`.
...
Sans accord, Four à Nu utilise le lecteur officiel, un lien au timecode, du texte
...
du Code de la propriété intellectuelle. Four à Nu ne construit pas son modèle sur
...
La [procédure d'examen Amazon](https://partenaires.amazon.fr/help/node/topic/G8TW5AE9XL2VX9VM) demande au moins trois ventes éligibles dans les 180 premiers jours et, pour un site, au moins dix contenus originaux substantiels. Toutes les propriétés et sources de trafic doivent être déclarées. Four à Nu ne doit donc pas candidater avec un bot X neuf comme propriété principale.
...
Ooni annonce aussi un taux de conversion supérieur à 2,5 % à partir de données 2023. C'est une affirmation du marchand, pas une mesure de Four à Nu. Les pages LUSINI présentent des formulations de taux partiellement contradictoires. Tout barème devra être capturé et confirmé dans le compte Awin avant une prévision.
...
La métrique centrale de Four à Nu sera donc le revenu par visite qualifiée, accompagné du coût complet et du nombre d'heures, pas le nombre d'articles publiés.
...
- La différenciation dépend de la qualité des rapprochements, des outils et des
  sources. Une simple réécriture de fiches marchandes ne suffit pas.
...
Four à Nu commence par le problème « choisir un four pour pizza napolitaine à domicile entre 300 et 1 500 euros ». Il n'élargit ni aux recettes généralistes ni au matériel professionnel avant d'avoir prouvé l'utilité et la conversion. Le professionnel reste une extension commerciale possible, pas une promesse du premier produit.

Si la synthèse documentaire n'apporte pas de valeur propre ou ne produit aucune
conversion après une itération mesurée, Four à Nu devra comparer une autre niche
avec le même score : valeur du panier, programme direct, intention d'achat,
accès aux sources, concurrence, risque réglementaire et potentiel de revenu
récurrent.
